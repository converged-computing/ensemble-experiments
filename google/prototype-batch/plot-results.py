...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=os.path.join(here, "final"),
    )
    parser.add_argument(
        "--out",
...
def main():
    ...
    # Find input results
    paths = list(recursive_find(indir, "results.json"))
    data = []
    pods_dfs = []
    all_dfs = {}
    ...
    for path in paths:
        run, name = path.split(os.sep)[-3:-1]
        run = run.replace('run', '')
        name = f"{name}-{run}"
        print(f"Parsing experiment {name}")
        parts = parse_experiment(path)
        if not parts:
            continue
        new_data, dfs = parts           
        data += new_data
...
def date_range(start, end, intv, name):
    ...
    # This is the interval length - let's set them to be the same this time
    diff = (end - start) / intv
...
def parse_experiment(results_json):
    """
    ...
    run, name = results_json.split(os.sep)[-3:-1]
    run = run.replace('run', '')
    name = f"{name}-{run}"
    indir = os.path.dirname(results_json)
    times_json = os.path.join(indir, "client-times.json")
    nodes_json = os.path.join(indir, "instance-activity.json")
    if not os.path.exists(times_json) or not os.path.exists(nodes_json):
        return
    ...
    intervals = date_range(str(earliest_time), latest_time, 100, name)
...
def plot_results(dfs, outdir):
    ...
    for experiment, res in dfs.items():
        ...
        # Squash experiment to be across runs
    ...
    pods_df['exp'] = ["-".join(x.split('-')[:-1]) for x in pods_df['experiment']]
    make_plot(
        pods_df,
